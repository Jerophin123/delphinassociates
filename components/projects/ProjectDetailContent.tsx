      ...
      <motion.div variants={itemVariants} className="w-full">
        <div className={`rounded-2xl sm:rounded-3xl border border-gray-100 ${tier === 'low' ? 'bg-white/95' : 'bg-white/80 backdrop-blur-sm'} shadow-sm p-4 sm:p-7`}>
          ...
          {detail?.highlights?.length ? (
            <motion.div
              variants={itemVariants}
              className="mt-6 grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-3"
            >
          ...
          {detail?.outcomes?.length ? (
            <motion.div
              variants={itemVariants}
              className="mt-6 rounded-2xl border border-accent/15 bg-accent/5 p-5 sm:p-6"
            >
              <h2 className="text-lg sm:text-xl font-semibold text-primary-dark">
                Outcomes & Impact
              </h2>
              <ul className="mt-3 space-y-2 text-sm sm:text-base text-gray-800">
                {detail.outcomes.map((outcome) => (
                  <li key={outcome} className="flex gap-3">
                    <span className="mt-1 h-2.5 w-2.5 rounded-full border border-accent/40 bg-accent/10" />
                    <span>{outcome}</span>
                  </li>
                ))}
              </ul>
