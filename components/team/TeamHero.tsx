      ...
      <div className="relative z-10 grid lg:grid-cols-[1fr_minmax(auto,400px)] gap-12 lg:gap-16 items-center max-w-6xl mx-auto">
        <div className="text-left">
          ...
          >
            ...
            <a
              href="#technical-team"
              className={`group inline-flex items-center justify-center gap-2 px-6 py-3 sm:px-8 sm:py-4 rounded-xl bg-[#fdfbf4] border border-gray-200 text-primary-dark font-bold text-sm sm:text-base hover:border-gray-300 hover:shadow-sm transition-all duration-300 transform hover:-translate-y-0.5 ${isHigh ? 'liquid-glass-btn-light-invert' : tier === 'mid' && !reducedMotion ? 'mid-glass-btn-light-invert' : ''}`}
            >
              Technical Team
            </a>
          </motion.div>
          ...
          {stats.map((stat, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative rounded-2xl border border-gray-100 ${tier === 'very-low' ? 'bg-[#fdfbf4]' : (tier === 'low' ? 'bg-[#fdfbf4]/95' : 'bg-[#fdfbf4]/60 backdrop-blur-md')} p-4 sm:p-6 transition-colors hover:border-accent/20 flex flex-col justify-center`}
              >
